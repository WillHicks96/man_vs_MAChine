#!/usr/bin/env python3
"""
Extended shortlist of 30 extreme halos per group (9 groups total):
  ...
  R1_andR2_andR3 — 30 most concordantly relaxed (smallest δ1 within R1∩R2∩R3)
  notR1_notR2_notR3 — 30 most concordantly disturbed (largest δ1 within ~R1∩~R2∩~R3)
"""
import os
import numpy as np
import h5py
...
OUT_FILE = os.path.join(EXPERIMENT_DIR, "halo_shortlist_30_all.txt")
N_SELECT = 30
...
# New: triple-concordant groups
R1_andR2_andR3    = R1 & (delta_2 < THR_D2) & (delta_3 < THR_KE)
notR1_notR2_notR3 = valid & (delta_1 >= THR_D1) & (delta_2 >= THR_D2) & (delta_3 >= THR_KE)

print(f"  R1_andR2_andR3    : N = {R1_andR2_andR3.sum():,}")
print(f"  notR1_notR2_notR3 : N = {notR1_notR2_notR3.sum():,}")
...
groups = [
    ...
    ("R1_andR2_andR3",    R1_andR2_andR3,    delta_1, True,  "delta_1",
     "smallest delta_1 (most concordantly relaxed: R1 AND R2 AND R3)"),
    ("notR1_notR2_notR3", notR1_notR2_notR3, delta_1, False, "delta_1",
     "largest delta_1 (most concordantly disturbed: not-R1 AND not-R2 AND not-R3)"),
]

# ── write ──────────────────────────────────────────────────────────────────────
with open(OUT_FILE, "w") as out:
    out.write("# Extended shortlist: 30 extreme halos per group (9 groups)\n")
    ...
    for name, mask, sort_arr, ascending, sort_col, extreme_desc in groups:
        ...
        print(f"  {name:22s}  {extreme_desc}")

print(f"\nDone. Extended shortlist written to:\n  {OUT_FILE}")
